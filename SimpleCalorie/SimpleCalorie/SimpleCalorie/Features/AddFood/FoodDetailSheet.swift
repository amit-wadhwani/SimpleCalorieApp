        ...
        Group {
            if isCustomAmountActive {
                ...
                Button {
                    ...
                    // Initialize with currently selected preset value
                    if let selectedServing = state.selectedServing {
                        // Use selected preset's domain amount
                        let domainAmount = gramsToDomainUnits(selectedServing.amountInGrams)
                        customAmountText = formatDomainUnitsForInput(domainAmount)
                    } else if let customGrams = state.customAmountInGrams, customGrams > 0 {
                        // Use existing custom amount if no preset selected
                        let domainAmount = gramsToDomainUnits(customGrams)
                        customAmountText = formatDomainUnitsForInput(domainAmount)
                    } else {
                        // Default based on domain: 1.0 for units, 100 for grams
                        if customDisplayUnit != nil {
                            customAmountText = "1"
                        } else if isGramBaseDomain {
                            customAmountText = "100"
                        } else {
                            customAmountText = "1"
                        }
                    }
                    isCustomAmountFocused = true
                    // Don't commit here - wait for user input
                    Haptics.light()
                } label: {
